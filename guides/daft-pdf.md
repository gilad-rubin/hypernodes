...
colab Run this tutorial on Google Colab ↗

thumbnail Follow along with the tutorial video ↗

👋 Hello and welcome to Daft ↗! This tutorial shows how to use Daft to create a typical PDF processing pipeline. By the end of the tutorial, we will have a fully functional pipeline that:

Starts with downloading PDFs from an S3
Extracts text boxes either using OCR or by reading the file format
Performs spatial layout analysis to group text boxes into either lines or paragraphs
Computes embeddings using a lightweight LLM, running locally
Saves everything as Parquet files
tl;dr: If you'd like, you can jump to the end to see the full pipeline.
...
To get a sense for what this data looks like, we can use the show method on the daft.DataFrame. This doesn't materialize the entire DataFrame. Instead, it only does enough computation to show us the first 10 rows.
...
Daft knows about URLs and has built-in support for downloading their contents! This is exposed via the .url.download() method on a column expression (that's the col('path')).
