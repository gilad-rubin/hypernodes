...
class ModelConfig:
    def __init__(self, model_name: str, temperature: float):
        ...
        self._api_key = "secret"     # Should not affect cache
...
# Different API keys don't affect cache (good for secrets)
config1._api_key = "key-123"
config2._api_key = "key-456"
assert config1.__cache_key__() == config2.__cache_key__()
```

**Best for**:

- Complex objects where you want explicit control over what affects caching
- Objects with secrets or private data that shouldn't affect cache
- Non-deterministic objects that need special handling
...
class SimpleConfig:
    batch_size: int
    learning_rate: float
    # Private attributes excluded from hash
    _internal_state: dict = None
    _cache: dict = None

# Two instances with same config share cache automatically
config1 = SimpleConfig(32, 0.001)
config2 = SimpleConfig(32, 0.001)
# These produce the same cache key even though they're different instances!
```

**Best for**:

- Simple data classes with straightforward serialization
- `@dataclass` decorated classes
- Configuration objects without complex nested structures
...
## Special Cases: Non-Deterministic Classes

**Problem**: Classes with random number generators or other non-deterministic behavior will produce different outputs even with the same configuration.

**Example of problematic class**:
```
...
self._rng = np.random.default_rng()  # Private, but non-deterministic!

def encode(self, text: str) -> Vector:

return self._rng.random(self.dim)  # Different every time!

```

**Solution 1: Make it deterministic with a seed**
```

class DeterministicEncoder:

def **init**(self, dim: int, seed: int = 42):
...
# Recreate RNG with seed each time for determinism

rng = np.random.default_rng(self.seed)

return rng.random(self.dim)

# These will cache correctly

encoder1 = DeterministicEncoder(dim=4, seed=42)

encoder2 = DeterministicEncoder(dim=4, seed=42)

# Same seed -> same outputs -> cache hit ✅

```

**Solution 2: Disable caching for non-deterministic nodes**
```

@node(output_name="random_embedding", cache=False)

def encode_randomly(encoder: RandomEncoder, text: str) -> Vector:

return encoder.encode(text)  # Always executes, never cached

```

**Key insight**: If your class has non-deterministic behavior, either:
1. Make it deterministic by including all randomness sources (seeds) in the cache key, or
2. Disable caching for nodes that use it with `cache=False`
```
...
Cache configuration follows the hierarchical inheritance system (see [Core Concepts](https://www.notion.so/Core-Concepts-4a4dd7402980462eb83fc2b3d5059ccc?pvs=21) for details).
