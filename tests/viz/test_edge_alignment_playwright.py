...
def generate_test_html(
    pipeline: Pipeline, depth: int = 99, separate_outputs: bool = True
) -> str:
    ...
    rf_data = renderer.render(
        graph_data, theme="dark", separate_outputs=separate_outputs, show_types=True
    )
...
class TestPlaywrightEdgeAlignment:
    ...
    def _click_pipeline_to_collapse(self, page, pipeline_id: str):
        ...
        # Find the pipeline's header/label area and click it (avoid inner node interception)
        # Try clicking the pipeline header which contains the collapse button
        header_selector = f'.react-flow__node[data-id="{pipeline_id}"] .cursor-pointer'
        if page.locator(header_selector).count() > 0:
            page.click(header_selector, force=True)
        else:
            # Fallback: force click on the pipeline node itself
            selector = f'.react-flow__node[data-id="{pipeline_id}"]'
            page.click(selector, force=True)
        # Wait for re-layout
        page.wait_for_timeout(500)

    def test_initial_render_valid_connections(
        self, browser_context, simple_nested_pipeline
    ):
    ...
    def test_collapse_maintains_valid_connections(
        self, browser_context, simple_nested_pipeline
    ):
        ...
        try:
            ...
            assert initial_validation["valid"], (
                f"Initial issues: {initial_validation['issues']}"
            )
            ...
            assert collapsed_validation["valid"], (
                f"Post-collapse issues: {collapsed_validation['issues']}"
            )
        finally:
            os.unlink(temp_path)

    def test_expand_maintains_valid_connections(
        self, browser_context, simple_nested_pipeline
    ):
        ...
        try:
            ...
            assert expanded_validation["valid"], (
                f"Post-expand issues: {expanded_validation['issues']}"
            )
        finally:
            os.unlink(temp_path)
