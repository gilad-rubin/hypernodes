...
from hypernodes import DiskCache, Pipeline, SeqEngine, node
...
def test_progress_basic_pipeline():
    ...
    engine = SeqEngine(callbacks=[progress])
    pipeline = Pipeline(nodes=[double, add_one], engine=engine, name="double_and_add")
...
def test_progress_with_map():
    ...
    engine = SeqEngine(callbacks=[progress])
    pipeline = Pipeline(nodes=[double, add_one], engine=engine, name="map_test")
...
def test_progress_with_cache():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        progress = ProgressCallback(enable=False)
        engine = SeqEngine(cache=cache, callbacks=[progress])
        pipeline = Pipeline(
            nodes=[double, add_one], engine=engine, name="cached_pipeline"
        )
...
def test_progress_with_map_and_cache():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        progress = ProgressCallback(enable=False)
        engine = SeqEngine(cache=cache, callbacks=[progress])
        pipeline = Pipeline(nodes=[double], engine=engine, name="map_cache_test")
...
def test_progress_with_nested_pipelines():
    ...
    outer_pipeline = Pipeline(
        nodes=[inner_pipeline.as_node(), add_one], engine=engine, name="outer"
    )
...
def test_progress_with_diamond_pattern():
    ...
    progress = ProgressCallback(enable=False)
    engine = SeqEngine(callbacks=[progress])
    pipeline = Pipeline(nodes=[double, triple, add], engine=engine, name="diamond")
...
def test_progress_with_multiple_map_params():
    ...
    progress = ProgressCallback(enable=False)
    engine = SeqEngine(callbacks=[progress])
    pipeline = Pipeline(nodes=[add], engine=engine, name="multi_param_map")

    results = pipeline.map(
        inputs={"x": [1, 2, 3], "y": [10, 20, 30]}, map_over=["x", "y"], map_mode="zip"
    )
...
def test_progress_with_fixed_and_varying_params():
    ...
    progress = ProgressCallback(enable=False)
    engine = SeqEngine(callbacks=[progress])
    pipeline = Pipeline(nodes=[multiply], engine=engine, name="fixed_varying")
...
def test_progress_enabled_visual():
    ...
    progress = ProgressCallback(enable=False)
    engine = SeqEngine(callbacks=[progress])
    pipeline = Pipeline(nodes=[double, add_one], engine=engine, name="visual_test")
...
def test_progress_map_visual():
    ...
    progress = ProgressCallback(enable=False)
    engine = SeqEngine(callbacks=[progress])
    pipeline = Pipeline(nodes=[square], engine=engine, name="map_visual_test")
