"""Tests for map operations with SeqEngine (default)."""
...
def test_map_product_mode():
    ...
    pipeline = Pipeline(nodes=[multiply])
    results = pipeline.map(
        inputs={"x": [2, 3], "y": [10, 100]}, map_over=["x", "y"], map_mode="product"
    )
...
def test_map_with_selective_output():
    ...
    results = pipeline.map(inputs={"x": [1, 2, 3]}, map_over="x", output_name="tripled")
