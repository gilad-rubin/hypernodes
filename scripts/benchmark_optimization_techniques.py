...
def benchmark_onebyone_vs_batch():
    ...
    num_texts = 100
    texts = [
        f"This is test passage number {i} with some text" for i in range(num_texts)
    ]
    ...
    print(
        f"   Time: {time_onebyone:.3f}s ({time_onebyone / num_texts * 1000:.1f}ms per text)"
    )
    ...
    print(
        f"   Time: {time_batch:.3f}s ({time_batch / num_texts * 1000:.1f}ms per text)"
    )
...
def benchmark_eager_vs_lazy():
    ...
    print("         but init is faster and serialization is MUCH faster!")
...
def benchmark_complete_retrieval():
    ...
    encode_passages_pipeline = Pipeline(
        nodes=[encode_passage_orig], name="encode_p_single"
    )
    encode_queries_pipeline = Pipeline(
        nodes=[encode_query_orig], name="encode_q_single"
    )
...
def main():
    ...
    print("   Benefit:    Faster startup, better serialization ⚡")
...
EXPECTED TOTAL SPEEDUP: 
   ...
   - After:  ~{(1000 * 0.01 + 100 * 0.01) / (batch_speedup * async_improvement):.2f}s
    ...
    print("   - outputs/RETRIEVAL_OPTIMIZATION_RECOMMENDATIONS.md")
    print("   - scripts/test_exact_repro_OPTIMIZED.py")
