#!/usr/bin/env python3
"""
Benchmark: SequentialEngine vs DaftEngine on retrieval pipeline
...
def main():
    ...
    print("SequentialEngine vs DaftEngine")
    print("=" * 70)
    print("\nBoth engines use:")
    ...
    print("\nDifference:")
    print("  - SequentialEngine: Simple, single-threaded")
    print("  - DaftEngine: Lazy execution, optimized for parallelism")
    print("=" * 70)
    
    results = []
    
    # Benchmark 1: SequentialEngine
    try:
        r1 = run_retrieval("SequentialEngine", use_daft_flag=False)
        results.append(r1)
    except Exception as e:
        print(f"❌ Failed: {e}")
        r1 = {"engine": "SequentialEngine", "total_time": None, "success": False}
        results.append(r1)
    ...
    for r in results:
        ...
        pipeline_str = f"{r.get('pipeline_time', 0):.2f}s" if r.get('pipeline_time') else "N/A"
        ndcg_str = f"{r.get('ndcg', 0):.4f}" if r.get('ndcg') else "N/A"
        print(f"{r['engine']:<25} {total_str:<15} {pipeline_str:<15} {ndcg_str:<10} {status}")
    ...
    print("\n✅ RECOMMENDATION:")
    print("   - Use SequentialEngine: Simple, reliable, fast enough")
