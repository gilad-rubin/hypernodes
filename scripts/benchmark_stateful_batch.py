...
def run_v1_sequential(
    texts: List[str], encoder: Any
...
def run_v2_hypernodes_daft(
    ...
    model_name = encoder._init_kwargs.get(
        "model_name", encoder._init_args[0] if encoder._init_args else "mock-model"
    )
...
def create_daft_sentence_transformer_encoder(
    max_concurrency: int = 2, use_process: bool = False
):
...
def verify_embeddings_match(
    emb1: List[List[float]], emb2: List[List[float]], tolerance: float = 1e-6
) -> bool:
...
def benchmark_mock_encoder(scale: int = 1000) -> dict:
    ...
    print(
        f"  Setup: {setup_time_v1:.3f}s | Exec: {exec_time_v1:.3f}s | Total: {total_v1:.3f}s"
    )
    ...
    try:
        ...
        if verify_embeddings_match(embeddings_v1, embeddings_v2):
            print("  ✓ Embeddings match V1")
        else:
            print("  ✗ WARNING: Embeddings don't match V1!")
        ...
        print(
            f"  Setup: {setup_time_v2:.3f}s | Exec: {exec_time_v2:.3f}s | Total: {total_v2:.3f}s | Speedup: {speedup_v2:.2f}x"
        )
    ...
    for config in daft_configs:
        ...
        try:
            ...
            print(
                f"  Setup: {setup_time_v3:.3f}s | Exec: {exec_time_v3:.3f}s | Total: {total_v3:.3f}s | Speedup: {speedup_v3:.2f}x"
            )
        except Exception as e:
            print(f"  ✗ FAILED: {e}")
            import traceback
