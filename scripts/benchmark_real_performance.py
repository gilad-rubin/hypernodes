...
class MockEncoderLazy:
    ...
    def _ensure_loaded(self):
        if self._model is None:
            print(f"  [LAZY] Loading {self.model_name} on first use... ", end="", flush=True)
...
def benchmark_batch_encoding():
    ...
    num_passages = 200
    passages = [{"uuid": f"p{i}", "text": f"passage text {i}"} for i in range(num_passages)]
    ...
    pipeline_batch = Pipeline(
        ...
        engine=SequentialEngine(),  # Simple execution
        name="batch"
    )
...
def benchmark_complete_retrieval():
    ...
    passage_results = pipeline_v1.map(
        inputs={"passage": passages, "encoder": encoder_v1},
        map_over="passage"
    )
    ...
    query_results = pipeline_v1_queries.map(
        inputs={"query": queries, "encoder": encoder_v1},
        map_over="query"
    )
    ...
    pipeline_v2 = Pipeline(
        nodes=[load_passages, load_queries, encode_passages_batch, encode_queries_batch],
    ...
    print(f"   Encoding passages and queries...")
    start = time.perf_counter()
    ...
    # Breakdown
    print(f"\n   Breakdown:")
    print(f"   - Passages: {time_passages_v1:.3f}s → ~{time_total_v2/2:.3f}s ({time_passages_v1/(time_total_v2/2):.1f}x)")
    print(f"   - Queries:  {time_queries_v1:.3f}s → ~{time_total_v2/2:.3f}s ({time_queries_v1/(time_total_v2/2):.1f}x)")
